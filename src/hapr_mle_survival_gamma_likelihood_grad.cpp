...
Rcpp::List hapr_mle_survival_gamma_nll_split_grad_cpp(
  ...
  const double* gc_censor_ptr = gc_censor.memptr();
  ...
  auto accumulate_obs = [&](double time,
    ...
    const double base = gamma_g * gc_value + xb;
    ...
    double d_gamma_w_sum = 0.0;

    for (int j = 0; j < kNumNodes; ++j) {
      const double weight = std::exp(val[j] - max_val) * inv_sum;
      const double dlinpred_weight = weight * dlinpred[j];
      const double dlinpred_dgamma_g = gc_value + post_c_over_a * kNodes[j];

      grad_ref[0] += dlinpred_weight * dlinpred_dgamma_g;

      if (is_weibull) {
        grad_ref[n_cols + 1] += weight * dlogk[j];
      }
      d_gamma_w_sum += dlinpred_weight;
    }

    for (int col = 0; col < n_cols; ++col) {
      grad_ref[1 + col] += d_gamma_w_sum * x_ptr[row_idx + n_rows * col];
    }
  };
