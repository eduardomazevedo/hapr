#' Test point estimates for exponential survival MLE across scenarios
#'
...
test_that("Exponential survival MLE point estimates are within 3 SE", {
  ...
  artifact_dir <- testthat::test_path("_artifacts", "mle_survival_exp")
  ...
  for (var_epsilon in VAR_EPSILON_VALUES) {
    for (n in N_VALUES) {
      scenario_name <- sprintf("exp_n%d_ve%.1f", as.integer(n), var_epsilon)
      ...
      expect_true(
        ...
        info = sprintf(
          ...
          scenario_name,
          paste(capture.output(print(comparison_table[!within_3se & !is.na(within_3se), ])),
                collapse = "\n")
        )
      )
