#' HAPR maximum likelihood estimation for survival models (gamma parameterization)
#'
#' @description
#' Fits the HAPR parametric survival models by maximum likelihood using
#' Gauss-Hermite quadrature, parameterized by gamma. Supported models are
...
#' @param start_gamma Named numeric vector for gamma parameters (gc, (Intercept), w1, ...)
...
#' @export
hapr_mle_survival_gamma <- function(
    event_time,
    event_status,
    gc,
    w,
  ...
  if (is.null(start_gamma) || !is.numeric(start_gamma)) {
    stop("start_gamma must be a numeric vector")
  }
  ...
  if (length(start_gamma) != nb) {
    stop(sprintf("start_gamma must have length %d.", nb))
  }
  names(start_gamma) <- gamma_names
  ...
  X_w <- add_intercept(w)
  neg_loglik <- make_hapr_mle_likelihood_survival_gamma_grad(
