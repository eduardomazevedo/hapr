aggregate_coverage_artifacts <- function(
  ...
  parse_scenario <- function(scenario) {
    result <- list(model_type = NA_character_, n = NA_integer_, var_epsilon = NA_real_, log_k = NA_real_)

    coverage_match <- regexec("^(lm|probit)_n([0-9]+)_ve([0-9.]+)$", scenario)
    ...
    survival_match <- regexec("^(exp|wei)_n([0-9]+)_ve([0-9.]+)_lk(-?[0-9.]+)$", scenario)
    ...
    if (length(survival_parts) > 0) {
      ...
      result$log_k <- as.numeric(survival_parts[5])
    }
